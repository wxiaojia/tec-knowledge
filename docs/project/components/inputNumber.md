...
## 问题；
1、无法阻止英文字符和中文字符的输入，但会在失焦后去除。

以下ant-desgin-vue的input-number的源码：
```js
...
// 解析值为 MiniDecimal 对象，如果无效返回 null，否则返回该小数对象。
...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    // Input by value
    /**
     * !newValue.equals(currentParsedValue)：如果 newValue 和 currentParsedValue 不相等，表示新的值和当前输入框中的值不一致，需要更新输入框的值。
        !userTypingRef.value：如果 userTypingRef.value 为 false，表示用户没有在输入，而是通过外部更新了 props.value，这时需要更新输入框的值。
        props.formatter：如果存在 formatter，说明用户可能自定义了输入格式，需要遵循自定义格式来更新值。
    */
    watch(
      () => props.value,
      () => {
...
```
onCompositionStart: 当用户开始使用输入法输入时（例如拼音输入法），触发 handleCompositionStart 方法，并设置 isComposing 标记为 true。
onCompositionEnd: 当输入法输入完成时（用户选择了字符），触发 handleCompositionEnd，并更新输入框的值。此时，我们可以放心地处理输入内容。

但输入后，会把值直接放入，没有阻止的操作；
...
## 解决：
放一个透明的input框，输入的时候用input,失焦的时候校验，把值放入input-number中
